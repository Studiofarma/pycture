...
def build_filter(args):
    if args.eq is None and args.gt is None and args.lt is None and args.neq is None:
        ...
        not_equals_filters = [pyf.NotEqualsFilter(neq[0], neq[1]) for neq in args.neq] if args.neq is not None else []
        ...
        filters = equals_filters + not_equals_filters + greater_filters + lesser_filters
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '--neq',  nargs=2, action='append',
        help='filter record by not equals. Example: --neq variable-name xx')
